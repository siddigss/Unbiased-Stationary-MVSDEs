...
samples = zeros(1, files_count*sample_count_per_file);
for file_index = 1:(length(folder_struct)-2)
    ...
    samples(:,(1+(file_index-1)*sample_count_per_file):(file_index*sample_count_per_file)) = A;
end



folder_name = 'results_6/times';
folder_struct = dir(folder_name);       % files will contain '.' and '..', so subtract 2 later.
files_count = length(folder_struct)-2;
sample_count_per_file = 1000;
samples_count = files_count*sample_count_per_file;
...
% hold on
% title('Meeting Time for Curie-Weiss');
% fontsize(14, "points");
% %set(gcf,'Color','w');
% xlabel('Meeting Time');
% ylabel('Percentage');
% hold off;
% %print(gcf, '-dpdf', 'figs5\CW_meeting_times_2.pdf')
...
xlabel('$\log_2$M')
ylabel('$\log_2$Time');
title('Average Run Time for Curie-Weiss');
fontsize(15, "points");
%set(gcf,'Color','w');
hold off;
export_fig figs/CW_mean_time.pdf -q101;
