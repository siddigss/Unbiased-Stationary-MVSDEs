...
true_density_function = @(x) 1/4.16575 * exp(-x.^4/2 + x.^2);
...
kernel_window = 0.25;
kernel_step = 0.1;
kernel_points = -3:kernel_step:4;
kernel_density_est = zeros(1, length(kernel_points));
for i = 1:length(kernel_points)
    phi = @(x) kernel((kernel_points(i) - x)/kernel_window)/kernel_window;
    e = 0;
...
title('Curie-Weiss Invariant Distribution');
fontsize(14, "points");
xlabel('$x$');
ylabel('$y$');
xlim([-3,4]);
